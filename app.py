...
# ─── TABS ──────────────────────────────────────────────────────
tab1, tab2, tab3, tab4 , tab5 = st.tabs(["🔍  Single URL Scanner", "📋  Batch Scanner", "📜  Scan History", "📊  Model & Workflow" , "🤖 Model Comparison"])
...
with tab1:
    ...
    if st.session_state.last_result:
        ...
        source = r.get("detection_label") or r.get("detection_source") or "UNKNOWN"
    ...
    def load_example(url):
        st.session_state["single_url"] = url
    ...
    if st.button("🚀 Scan All URLs", key="batch_scan"):
        ...
        if not urls:
            ...
            for r in results:
                ...
                src = (r.get("detection_source") or r.get("detection_label") or "UNKNOWN")[:18]
                st.markdown(f"""
                <div class="hist-row">
...
with tab4:
    ...
    """, unsafe_allow_html=True)
# ═══════════════════════════════════════════════════════════════
# TAB 5 — Model Comparison Dashboard (ENHANCED UI)
# ═══════════════════════════════════════════════════════════════
...
with tab5:
    ...
    .cm-cell .num { font-size: 22px; font-weight: 800; }
    .cm-cell .lbl { font-size: 10px; text-transform: uppercase; letter-spacing: 0.08em; margin-top: 4px; }
    .cm-axis { text-align: center; font-size: 11px; color: #475569; font-family: 'JetBrains Mono', monospace; letter-spacing: 0.06em; }
    .cm-col-labels { display: grid; grid-template-columns: 1fr 1fr; max-width: 300px; margin: 0 auto; }
    .cm-col-label { text-align: center; font-size: 11px; color: #475569; font-family: 'JetBrains Mono', monospace; padding: 4px; }
    ...
    .score-table tr:hover td { background: #0f1923; }
    .bar-bg { background: #1e293b; border-radius: 4px; height: 5px; width: 100%; margin-top: 4px; }
    ...
    <div class="dash-header">
        ...
        <p>Phishing detection &nbsp;·&nbsp; 6 classifiers &nbsp;·&nbsp; 109,600 test samples</p>
    </div>
    """, unsafe_allow_html=True)

    # ── 📊 DATA ──────────────────────────────────────────────────
    models      = ["Random Forest", "XGBoost", "Decision Tree", "Logistic Reg", "Grad Boost", "KNN"]
    short_names = ["RF", "XGBoost", "DTree", "LogReg", "GBoost", "KNN"]
    accuracy    = [96.8, 97.4, 94.1, 92.3, 95.9, 89.7]
    precision   = [96.2, 97.1, 93.5, 91.8, 95.4, 88.9]
    recall      = [97.1, 97.7, 94.8, 92.9, 96.3, 90.2]
    f1_score    = [96.6, 97.4, 94.1, 92.3, 95.8, 89.5]
    colors_hex  = ["#3266ad", "#2ea37d", "#c0703a", "#8a5bb5", "#a33333", "#73726c"]
    ...
    def styled_axes(fig, xaxis=True, yaxis=True):
        updates = {}
        if xaxis:
            updates["xaxis"] = dict(gridcolor=GRID_COLOR, linecolor="#1e293b", tickcolor="#1e293b")
        if yaxis:
            updates["yaxis"] = dict(gridcolor=GRID_COLOR, linecolor="#1e293b", tickcolor="#1e293b")
    ...
    <div class="metric-row">
        <div class="metric-card highlight">
            <div class="m-label">Best Model</div>
            <div class="m-val">97.4%<span class="badge-best">CHAMPION</span></div>
            <div class="m-sub">XGBoost</div>
        </div>
        <div class="metric-card">
            <div class="m-label">Avg Accuracy</div>
            <div class="m-val">94.4%</div>
            <div class="m-sub">across 6 models</div>
        </div>
        <div class="metric-card">
            <div class="m-label">Test Samples</div>
            <div class="m-val">109.6K</div>
            <div class="m-sub">legit + phishing</div>
        </div>
        <div class="metric-card">
            <div class="m-label">Accuracy Gap</div>
            <div class="m-val">7.7pp</div>
            <div class="m-sub">best vs worst</div>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    fig_bar.update_layout(
        **base_layout(height=280),
        xaxis_title=None,
        yaxis=dict(range=[85, 100], ticksuffix="%", gridcolor=GRID_COLOR, linecolor="#1e293b"),
        xaxis=dict(gridcolor=GRID_COLOR, linecolor="#1e293b"),
        showlegend=False
    )
    st.plotly_chart(fig_bar, use_container_width=True)

    # ── 📈 ROC + 🕸️ RADAR ────────────────────────────────────────
    st.markdown('<div class="section-title">ROC Curves &amp; Radar Profile</div>', unsafe_allow_html=True)

    col_roc, col_radar = st.columns(2)
    ...
    with col_roc:
        ...
        roc_data = [
            ("XGBoost",         np.minimum(1, np.sqrt(fpr) * 1.02),   "#3266ad", 2.5),
            ("Random Forest",   np.minimum(1, fpr ** 0.78),            "#2ea37d", 2.0),
            ("Logistic Reg",    np.minimum(1, fpr ** 0.88),            "#c0703a", 2.0),
        ]
        for name, tpr, col, width in roc_data:
            fig_roc.add_trace(go.Scatter(
                x=fpr, y=tpr, mode="lines", name=name,
                line=dict(color=col, width=width),
                hovertemplate=f"<b>{name}</b><br>FPR: %{{x:.2f}}<br>TPR: %{{y:.2f}}<extra></extra>"
            ))
        fig_roc.add_trace(go.Scatter(
        ...
        fig_roc.update_layout(
            **base_layout(height=300),
            xaxis=dict(title="False Positive Rate", range=[0, 1], gridcolor=GRID_COLOR, linecolor="#1e293b", dtick=0.2),
            yaxis=dict(title="True Positive Rate",  range=[0, 1], gridcolor=GRID_COLOR, linecolor="#1e293b", dtick=0.2),
            legend=dict(bgcolor="rgba(0,0,0,0)", font=dict(size=10))
        )
        st.plotly_chart(fig_roc, use_container_width=True)

    with col_radar:
        radar_cats = ["Accuracy", "Precision", "Recall", "F1", "Speed", "Interpret."]
        fig_radar = go.Figure()

        radar_models = [
            ("XGBoost",       [97.4, 97.1, 97.7, 97.4, 72, 65],  "#3266ad", "rgba(50,102,173,0.15)"),
            ("Random Forest", [96.8, 96.2, 97.1, 96.6, 60, 75],  "#2ea37d", "rgba(46,163,125,0.12)"),
        ]
        for name, vals, col, fill in radar_models:
        ...
        fig_radar.update_layout(
            **base_layout(height=300),
            polar=dict(
                bgcolor=PLOT_BG,
                radialaxis=dict(visible=True, range=[50, 100], gridcolor=GRID_COLOR, tickfont=dict(size=9)),
                angularaxis=dict(gridcolor=GRID_COLOR, linecolor="#1e293b")
            ),
            legend=dict(bgcolor="rgba(0,0,0,0)", font=dict(size=10))
        )
    ...
    st.markdown('<div class="section-title">Confusion Matrix &amp; Performance Scorecard</div>', unsafe_allow_html=True)

    col_cm, col_score = st.columns(2)

    with col_cm:
        st.markdown("""
        <div class="cm-wrapper">
            <div class="cm-axis" style="margin-bottom:6px">Predicted →</div>
            <div class="cm-col-labels">
                <div class="cm-col-label">Legit</div>
                <div class="cm-col-label">Phishing</div>
            </div>
            <div class="cm-grid">
                <div class="cm-cell" style="background:#0c1e35;border:1px solid #1e3a5f">
                    <div class="num" style="color:#93c5fd">53,000</div>
                    <div class="lbl" style="color:#3b82f6">True Neg</div>
                </div>
                <div class="cm-cell" style="background:#2d0c0c;border:1px solid #5f1e1e">
                    <div class="num" style="color:#fca5a5">1,800</div>
                    <div class="lbl" style="color:#ef4444">False Pos</div>
                </div>
                <div class="cm-cell" style="background:#2d1f05;border:1px solid #5f3b0a">
                    <div class="num" style="color:#fcd34d">1,200</div>
                    <div class="lbl" style="color:#f59e0b">False Neg</div>
                </div>
                <div class="cm-cell" style="background:#052d1a;border:1px solid #064e2c">
                    <div class="num" style="color:#6ee7b7">53,600</div>
                    <div class="lbl" style="color:#10b981">True Pos</div>
                </div>
            </div>
            <div class="cm-axis" style="margin-top:6px">← Actual</div>
        </div>
        """, unsafe_allow_html=True)

    with col_score:
        sorted_data = sorted(zip(models, accuracy, precision, recall, f1_score), key=lambda x: -x[1])
        rows = ""
        for m, a, p, r, f in sorted_data:
            pct = int((a - 89) / (97.4 - 89) * 100)
            is_best = m == "XGBoost"
            badge = '<span class="badge-best">best</span>' if is_best else ""
            bar_color = "#3266ad" if is_best else "#334155"
            rows += f"""
            <tr>
                <td style="font-weight:{'600' if is_best else '400'};color:#{'e2e8f0' if is_best else 'cbd5e1'}">{m}{badge}</td>
                <td style="font-family:'JetBrains Mono',monospace;font-size:12px">{a:.1f}%</td>
                <td style="width:100px">
                    <div style="font-size:9px;color:#475569;font-family:'JetBrains Mono',monospace">{pct}%</div>
                    <div class="bar-bg"><div class="bar-fill" style="width:{pct}%;background:{bar_color}"></div></div>
                </td>
            </tr>"""
        st.markdown(f"""
        <table class="score-table">
            <thead><tr><th>Model</th><th>Accuracy</th><th>Score</th></tr></thead>
            <tbody>{rows}</tbody>
        </table>
        """, unsafe_allow_html=True)

    # ── 📊 GROUPED BAR — ALL METRICS ────────────────────────────
    st.markdown('<div class="section-title">Precision / Recall / F1 by Model</div>', unsafe_allow_html=True)

    fig_group = go.Figure()
    metric_defs = [
        ("Accuracy",  accuracy,  "#3266ad"),
        ("Precision", precision, "#2ea37d"),
        ("Recall",    recall,    "#c0703a"),
        ("F1 Score",  f1_score,  "#8a5bb5"),
    ]
    for name, vals, col in metric_defs:
        fig_group.add_trace(go.Bar(
            name=name,
            x=short_names,
            y=vals,
            marker=dict(color=col, cornerradius=4, line=dict(width=0)),
    ...
    fig_group.update_layout(
        **base_layout(height=300),
        ...
        xaxis=dict(gridcolor=GRID_COLOR, linecolor="#1e293b"),
        yaxis=dict(range=[85, 100], ticksuffix="%", gridcolor=GRID_COLOR, linecolor="#1e293b"),
        legend=dict(bgcolor="rgba(0,0,0,0)", orientation="h", yanchor="bottom", y=1.02, font=dict(size=10))
    )
    st.plotly_chart(fig_group, use_container_width=True)

    # ── 📉 HEATMAP — METRIC MATRIX ───────────────────────────────
    st.markdown('<div class="section-title">Metric Heatmap</div>', unsafe_allow_html=True)

    heat_z = [accuracy, precision, recall, f1_score]
    heat_y = ["Accuracy", "Precision", "Recall", "F1"]
    text_z = [[f"{v:.1f}%" for v in row] for row in heat_z]
    ...
    fig_heat = go.Figure(go.Heatmap(
        ...
        colorscale=[[0, "#0c1e35"], [0.5, "#1e3a5f"], [1, "#3266ad"]],
        showscale=True,
        zmin=88, zmax=98,
        colorbar=dict(
            ticksuffix="%",
    ...
    fig_heat.update_layout(
        **base_layout(height=220),
        xaxis=dict(side="bottom", linecolor="#1e293b"),
        yaxis=dict(linecolor="#1e293b")
    )
